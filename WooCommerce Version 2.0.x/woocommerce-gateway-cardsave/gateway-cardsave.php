...
function init_your_gateway() {
	...
	class woocommerce_cardsave extends WC_Payment_Gateway {
		...
		public function __construct() { 
			...
			// Actions
			add_action('woocommerce_api_woocommerce_cardsave', array(&$this, 'check_cardsave_response'));			
		...
		public function admin_options() {
			...
			<p><b>Module Version:</b> 2.0 (For WooCommerce v2.0+)<br />
			<b>Module Date:</b> 14 March 2013</p>
		...
		public function generate_cardsave_form( $order_id ) {
			...
			$CustomerName = stripGWInvalidChars($order->billing_first_name . " " . $order->billing_last_name);
			$Address1 =  stripGWInvalidChars($order->billing_address_1);
			$Address2 =  stripGWInvalidChars($order->billing_address_2);
			$Address3 =  stripGWInvalidChars($order->billing_address_3);
			$Address4 =  stripGWInvalidChars($order->billing_address_4);
			$City =  stripGWInvalidChars($order->billing_city);
			$State =  stripGWInvalidChars($order->billing_state);
			$PostCode =  stripGWInvalidChars($order->billing_postcode);
			$EmailAddress =  stripGWInvalidChars($order->billing_email);
			$PhoneNumber =  stripGWInvalidChars($order->billing_phone);
			...
			$HashDigest=$HashDigest . '&EchoAVSCheckResult=' . 'true';
			$HashDigest=$HashDigest . '&EchoCV2CheckResult=' . 'true';
			$HashDigest=$HashDigest . '&EchoThreeDSecureAuthenticationCheckResult=' . 'true';
			$HashDigest=$HashDigest . '&EchoCardType=' . 'true';
			...
			$HashDigest=$HashDigest . '&CustomerName=' . $CustomerName;
			$HashDigest=$HashDigest . '&Address1=' . $Address1;
			$HashDigest=$HashDigest . '&Address2=' . $Address2;
			$HashDigest=$HashDigest . '&Address3=' . $Address3;
			$HashDigest=$HashDigest . '&Address4=' . $Address4;
			$HashDigest=$HashDigest . '&City=' . $City;
			$HashDigest=$HashDigest . '&State=' . $State;
			$HashDigest=$HashDigest . '&PostCode=' . $PostCode;
			$HashDigest=$HashDigest . '&CountryCode=' . $this->countryISO;			
			$HashDigest=$HashDigest . '&EmailAddress=' . $EmailAddress;
			$HashDigest=$HashDigest . '&PhoneNumber=' . $PhoneNumber;
			$HashDigest=$HashDigest . '&EmailAddressEditable=' . 'false';
			$HashDigest=$HashDigest . '&PhoneNumberEditable=' . 'false';			
			...
			$HashDigest=$HashDigest . "&ServerResultURL=" . trailingslashit(get_bloginfo('wpurl')).'?wc-api=woocommerce_cardsave';
			...
			$cardsave_args = array(
				...
				'ServerResultURL' 			=> trailingslashit(get_bloginfo('wpurl')).'?wc-api=woocommerce_cardsave',
				'PaymentFormDisplaysResult' 			=> "FALSE",
				'ServerResultURLCookieVariables' 		=> "",
				'ServerResultURLFormVariables' 			=> "",
				'ServerResultURLQueryStringVariables' 	=> "",
				'ThreeDSecureCompatMode' 	=> "false",
				'ServerResultCompatMode' 	=> "false"
			);
		...
		**/
		function check_cardsave_response() {
			...
			//if (isset($_GET['cardsave_listener']) && $_GET['cardsave_listener'] == 'cardsave_server_callback') {
				...
				try
					{
						...
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [CardType] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['CardType'] = $_POST["CardType"];
						}
						// card class
						if (!isset($_POST["CardClass"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [CardClass] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['CardClass'] = $_POST["CardClass"];
						}
						// card issuer
						if (!isset($_POST["CardIssuer"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [CardIssuer] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['CardIssuer'] = $_POST["CardIssuer"];
						}
						// card issuer
						if (!isset($_POST["CardIssuerCountryCode"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [CardIssuerCountryCode] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['CardIssuerCountryCode'] = $_POST["CardIssuerCountryCode"];
						}
						// address numeric check
						if (!isset($_POST["AddressNumericCheckResult"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [AddressNumericCheckResult] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['AddressNumericCheckResult'] = $_POST["AddressNumericCheckResult"];
						}
						// post code check
						if (!isset($_POST["PostCodeCheckResult"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [PostCodeCheckResult] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['PostCodeCheckResult'] = $_POST["PostCodeCheckResult"];
						}
						// CV2 check
						if (!isset($_POST["CV2CheckResult"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [CV2CheckResult] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['CV2CheckResult'] = $_POST["CV2CheckResult"];
						}
						// 3DS check
						if (!isset($_POST["ThreeDSecureAuthenticationCheckResult"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [ThreeDSecureAuthenticationCheckResult] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$cardsave_response['ThreeDSecureAuthenticationCheckResult'] = $_POST["ThreeDSecureAuthenticationCheckResult"];
						}
						...
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [EmailAddress] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$$cardsave_response['szOutputMessage'] = $_POST["EmailAddress"];
						}
						// phone number
						if (!isset($_POST["PhoneNumber"]))
						{
							$szOutputMessage = $szOutputMessage . "Expected variable [PhoneNumber] not received";
							$boErrorOccurred = true;
						}
						else
						{
							$$cardsave_response['szOutputMessage'] = $_POST["PhoneNumber"];
						}
				...
				$calchash=$calchash . '&AddressNumericCheckResult=' . $_POST["AddressNumericCheckResult"];
				$calchash=$calchash . '&PostCodeCheckResult=' . $_POST["PostCodeCheckResult"];
				$calchash=$calchash . '&CV2CheckResult=' . $_POST["CV2CheckResult"];
				$calchash=$calchash . '&ThreeDSecureAuthenticationCheckResult=' . $_POST["ThreeDSecureAuthenticationCheckResult"];
				$calchash=$calchash . '&CardType=' . $_POST["CardType"];
				$calchash=$calchash . '&CardClass=' . $_POST["CardClass"];
				$calchash=$calchash . '&CardIssuer=' . $_POST["CardIssuer"];
				$calchash=$calchash . '&CardIssuerCountryCode=' . $_POST["CardIssuerCountryCode"];
				...
				$calchash=$calchash . '&CustomerName=' . stripslashes($_POST["CustomerName"]);
				$calchash=$calchash . '&Address1=' . stripslashes($_POST["Address1"]);
				$calchash=$calchash . '&Address2=' . stripslashes($_POST["Address2"]);
				$calchash=$calchash . '&Address3=' . stripslashes($_POST["Address3"]);
				$calchash=$calchash . '&Address4=' . stripslashes($_POST["Address4"]);
				$calchash=$calchash . '&City=' . stripslashes($_POST["City"]);
				$calchash=$calchash . '&State=' . stripslashes($_POST["State"]);
				$calchash=$calchash . '&PostCode=' . stripslashes($_POST["PostCode"]);
				$calchash=$calchash . '&CountryCode=' . $_POST["CountryCode"];
				$calchash=$calchash . '&EmailAddress=' . $_POST["EmailAddress"];
				$calchash=$calchash . '&PhoneNumber=' . $_POST["PhoneNumber"];
				...
				if($nOutputProcessedOK == 30) {
					wp_die("StatusCode=".$nOutputProcessedOK."&Message=".$szOutputMessage,"",array( 'response' => 500 ));
				} else {
		...
		function successful_request($cardsave_response) {
			...
			if (isset($cardsave_response)) {
					...
					if ($transauthorised == true) {
						...
						if ($order->status !== 'completed') {
							if ($order->status == 'processing') {
								...
								$order->add_order_note('CardSave Payment: SUCCESSFUL | '. $cardsave_response['szMessage'] .' | Transaction Cross Reference: ' . $cardsave_response['szCrossReference'] . ' | Address Check: ' . $cardsave_response['AddressNumericCheckResult'] . ' | Postcode Check: ' . $cardsave_response['PostCodeCheckResult'] . ' | CV2 Check: ' . $cardsave_response['CV2CheckResult'] . ' | 3DS: ' . $cardsave_response['ThreeDSecureAuthenticationCheckResult']);								
						...
						//Check Security checks for failure
						$failurereasons = "";
						if ($cardsave_response['AddressNumericCheckResult'] == "FAILED") {
							$failurereasons .= "Address check failed. ";
						}
						if ($cardsave_response['PostCodeCheckResult'] == "FAILED") {
							$failurereasons .= "Postcode check failed. ";
						}
						if ($cardsave_response['CV2CheckResult'] == "FAILED") {
							$failurereasons .= "CV2 number is incorrect. ";
						}
						if ($cardsave_response['ThreeDSecureAuthenticationCheckResult'] == "FAILED") {
							$failurereasons .= "Verified by Visa / MasterCard SecureCode check failed. ";
						}						
						
						//Add admin order note
						$order->add_order_note('CardSave Payment: FAILED | Failure Message: '. $cardsave_response['szMessage'] .' | Transaction Cross Reference: ' . $cardsave_response['szCrossReference'] . ' | Address Check: ' . $cardsave_response['AddressNumericCheckResult'] . ' | Postcode Check: ' . $cardsave_response['PostCodeCheckResult'] . ' | CV2 Check: ' . $cardsave_response['CV2CheckResult'] . ' | 3DS: ' . $cardsave_response['ThreeDSecureAuthenticationCheckResult']);
						//Add customer order note
						$order->add_order_note('Payment Failed - ' . $cardsave_response['szMessage'] . '. ' .$failurereasons,'customer');
					}
				} catch (Exception $e) {
					$nOutputProcessedOK = 30;
				...
				wp_die("StatusCode=".$nOutputProcessedOK."&Message=".$szOutputMessage,"",array( 'response' => 200 ));
			}
		}	
